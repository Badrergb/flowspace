...
def test_sync_upsert(auth_client, test_db, test_user_and_token):
    ...
    # Verify in DB
    task = test_db.query(Task).filter(Task.id == uuid.UUID(entity_id)).first()
...
def test_lww_conflict_resolution(auth_client, test_db, test_user_and_token):
    ...
    task = Task(id=uuid.UUID(entity_id), user_id=user.id, title="Initial", version=initial_version)
    test_db.add(task)
    test_db.commit()
...
def test_sync_soft_delete(auth_client, test_db, test_user_and_token):
    ...
    task = Task(id=uuid.UUID(entity_id), user_id=user.id, title="To Delete", version=int(time.time_ns()))
    test_db.add(task)
    test_db.commit()
